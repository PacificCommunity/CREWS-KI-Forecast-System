...
def gen_tide(now):
    ...
    then = now + dt.timedelta(days=9.5,minutes=1)
    time_tide = mdates.drange(now,then,dt.timedelta(minutes=1))
    time_tide_hourly = mdates.drange(now,then,dt.timedelta(hours=1))
    ...
    # ax1.plot(date_time_hourly,TIDE_hourly.data,'k*')
