...
def par_run(now):
    ...
    cygfolder = '/cygdrive/d/CREWS_KI/operational_v1/runs/' + now.strftime("%Y") + now.strftime("%m") + now.strftime("%d") +  now.strftime("%H")  +'/'
    ...
    # cygfolder = '/cygdrive/c/Killo/Tarawa_operational/python/runs/' + now.strftime("%Y") + now.strftime("%m") + now.strftime("%d") + '00/'
    # for statement in execute('C:/Killo/cygwin64/bin/bash --login -c "cd %s;  /home/executables/adcprep --np 4 --partmesh"' % cygfolder):
    #     print(statement, end="") 
    # for statement in execute('C:/Killo/cygwin64/bin/bash --login -c "cd %s;  /home/executables/adcprep --np 4 --prepall"' % cygfolder):
    #     print(statement, end="")
    ...
    # for statement in execute('C:/Killo/cygwin64/bin/bash --login -c   "cd %s; mpirun -np 4  /home/executables/punswan"' % cygfolder ):
    #     print(statement, end="")
    os.system('C:/cygwin64/bin/bash --login -c "cd %s; mpirun --use-hwthread-cpus -np  16 /home/executables/punswan_4131"' % cygfolder )
    try: 
        os.system('taskkill /F /IM punswan.exe')
    except:
