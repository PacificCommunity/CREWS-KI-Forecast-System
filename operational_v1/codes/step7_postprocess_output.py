...
def plot_Hs_Dir_maps(nat,tar,dtt,x,y,triang,Time,Hs,Dir,result_folder):
    ...
    for ts in range(24,len(Hs),dtt):  
        ...
        if nat==1:
            ...
            plt.title(Time[ts])
            fileprint=result_folder + 'Kiri_' + Time[ts]
        ...
        if tar==1:
            ...
            plt.title(Time[ts])
            fileprint=result_folder + 'Tara_' + Time[ts]
            plt.savefig(fileprint)
            plt.close(fig2)
...
def SWAN2nc(x,y,triang,time_waves,Hs,Tm,Tp,Dir,Windv_x,Windv_y,xll,yll,xur,yur,inc,namenc):
    ...
    for ts in range(48,len(time_waves)):# change the starting hour into range to the lenght of the spin-up 
        ...
        WindDirp[ts,:,:] = 270 - np.arctan2(wy,wx)*180/np.pi
...
def postprocess_SWAN(now,plot):
    ...
    ## Mesh grid values for interpolation national scale Gilbert grid boundary limit coordinates
    xll = 172.06
    yll = -2.15
    xur = 176.6
    yur = 3.55
    inc =0.01# about1km
    namenc = out_name  + '/results/Gilbert_Group.nc'  
    SWAN2nc(x,y,triang,time_waves,Hs,Tm,Tp,Dir,Windv_x,Windv_y,xll,yll,xur,yur,inc,namenc)
    
    
    ## Mesh grid values for interpolation island scale Tarawa grid boundary limit coordinates 
    xll = 172.7
    yll = 1.2
    xur = 173.3
    yur = 1.7
    inc =0.001# about1km
    namenc = out_name  + '/results/Tarawa.nc'  
    SWAN2nc(x,y,triang,time_waves,Hs,Tm,Tp,Dir,Windv_x,Windv_y,xll,yll,xur,yur,inc,namenc)
